...
def listen_print_loop(responses):
    ...
    for response in responses:
        ...
        overwrite_chars = ' ' * (num_chars_printed - len(result_transcript))
        sys.stdout.write(result_transcript + overwrite_chars + '\r')
        sys.stdout.flush()
        num_chars_printed = len(result_transcript)

        if last_transcript != result_transcript:
            # 새로운 음성 입력일 때만 출력
            print(result_transcript)
            last_transcript = result_transcript
            num_chars_printed = 0
        if re.search(r'\b(exit|quit)\b', result_transcript, re.I):
            print('Exiting..')
            break
...
def main():
    ...
    with MicrophoneStream(RATE, CHUNK) as stream:
        ...
        while True:
            ...
            listen_print_loop(responses)
            # 새로운 음성 입력을 받기 위해 반복
            # 기존 responses를 사용한 후 다시 음성 입력을 받도록 반복
